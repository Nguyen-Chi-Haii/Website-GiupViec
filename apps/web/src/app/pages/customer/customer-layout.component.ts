...
@Component({
      ...
      <aside class="sidebar" [class.collapsed]="sidebarCollapsed()">
        ...
        <nav class="sidebar-nav">
          <a routerLink="/customer" routerLinkActive="active" [routerLinkActiveOptions]="{exact: true}" class="nav-item">
            ...
            @if (!sidebarCollapsed()) { <span>Đơn Của Tôi</span> }
          </a>
          <a routerLink="/customer/profile" routerLinkActive="active" class="nav-item">
            <span class="material-symbols-outlined">person</span>
            @if (!sidebarCollapsed()) { <span>Hồ Sơ</span> }
          </a>
          <a routerLink="/booking" class="nav-item book-now">
            <span class="material-symbols-outlined">add_circle</span>
            @if (!sidebarCollapsed()) { <span>Đặt Dịch Vụ</span> }
          </a>
        </nav>
  ...
  styles: [`
    ...
    .nav-item.book-now { background: #13b9a5; color: white; margin-top: 1rem; flex-shrink: 0; }
    .nav-item.book-now:hover { background: #0f9685; }
    ...
    .user-avatar { width: 36px; height: 36px; border-radius: 50%; background: linear-gradient(135deg, #1d4ed8 0%, #1e40af 100%); display: flex; align-items: center; justify-content: center; color: white; font-weight: 600; font-size: 0.8rem; }
